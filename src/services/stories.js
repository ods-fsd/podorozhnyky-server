...
export const getSavedStories = async (userId, page = 1, perPage = 10) => {
  ...
  //  Якщо користувача фізично немає в БД — викидаємо помилку 404
  if (!user) {
  ...
  const query = StoriesCollection.find({ _id: { $in: user.favorites } });

  // Отримуємо дані з пагінацією та підрахунком
  const [storiesCount, stories] = await Promise.all([
    StoriesCollection.countDocuments({ _id: { $in: user.favorites } }),
    query
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
